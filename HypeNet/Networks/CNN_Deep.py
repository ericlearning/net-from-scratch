...
'''
[Architecture]
	Input -> Conv(16,3,3) -> Relu -> Conv(16,3,3) -> Relu -> Pool
		  -> Conv(32,3,3) -> Relu -> Conv(32,3,3) -> Relu -> Pool
		  -> Conv(64,3,3) -> Relu -> Conv(64,3,3) -> Relu -> Pool
		  -> Affine -> Relu -> Dropout -> Affine -> Softmax
'''
class CNN_Deep:
	def __init__(self, input_dim = (1, 28, 28), conv_param1 = {'filter_num' : 16, 'filter_height' : 3, 'filter_width' : 3, 'pad' : 1, 'stride' : 1}
								, conv_param2 = {'filter_num' : 16, 'filter_height' : 3, 'filter_width' : 3, 'pad' : 1, 'stride' : 1}
								, conv_param3 = {'filter_num' : 32, 'filter_height' : 3, 'filter_width' : 3, 'pad' : 1, 'stride' : 1}
								, conv_param4 = {'filter_num' : 32, 'filter_height' : 3, 'filter_width' : 3, 'pad' : 2, 'stride' : 1}
								, conv_param5 = {'filter_num' : 64, 'filter_height' : 3, 'filter_width' : 3, 'pad' : 1, 'stride' : 1}
								, conv_param6 = {'filter_num' : 64, 'filter_height' : 3, 'filter_width' : 3, 'pad' : 1, 'stride' : 1}, hidden_size = 200, output_size = 10):
	...
	def calculate_final_conv_output_size(self):
		...
		final_conv_output_H = pool3_output_H
		final_conv_output_W = pool3_output_W

		return final_conv_output_H, final_conv_output_W

	def paramInit(self):
		(final_conv_output_H, final_conv_output_W), final_conv_output_C = self.calculate_final_conv_output_size(), self.conv_param6['filter_num']		#(4, 4, 64)
		...
		self.params['W7'] = np.random.randn(prev_connect_num[7], prev_connect_num[6]) * np.sqrt(2.0 / prev_connect_num[6])
		self.params['b7'] = np.zeros((prev_connect_num[7], 1))
		self.params['W8'] = np.random.randn(self.output_size, prev_connect_num[7]) * np.sqrt(2.0 / prev_connect_num[7])
		self.params['b8'] = np.zeros((self.output_size, 1))
