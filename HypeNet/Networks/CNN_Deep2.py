...
'''
class CNN_Deep2:
	def __init__(self, input_dim = (3, 64, 64), conv_param1 = {'filter_num' : 32, 'filter_height' : 3, 'filter_width' : 3, 'pad' : 1, 'stride' : 1}
								...
								, conv_param8 = {'filter_num' : 256, 'filter_height' : 3, 'filter_width' : 3, 'pad' : 1, 'stride' : 1}, hidden_size_1 = 256, hidden_size_2 = 256, output_size = 2):
	...
	def calculate_final_conv_output_size(self):
		...
		final_conv_output_H = pool4_output_H
		final_conv_output_W = pool4_output_W

		return final_conv_output_H, final_conv_output_W

	def paramInit(self):
		(final_conv_output_H, final_conv_output_W), final_conv_output_C = self.calculate_final_conv_output_size(), self.conv_param8['filter_num']		#(4, 4, 128)
		...
		self.params['W9'] = np.random.randn(prev_connect_num[9], prev_connect_num[8]) * np.sqrt(2.0 / prev_connect_num[8])
		self.params['b9'] = np.zeros((prev_connect_num[9], 1))
		self.params['W10'] = np.random.randn(prev_connect_num[10], prev_connect_num[9]) * np.sqrt(2.0 / prev_connect_num[9])
		self.params['b10'] = np.zeros((prev_connect_num[10], 1))
		self.params['W11'] = np.random.randn(self.output_size, prev_connect_num[10]) * np.sqrt(2.0 / prev_connect_num[10])
		self.params['b11'] = np.zeros((self.output_size, 1))
	...
	def layerInit(self):
		...
		self.layers['Affine1'] = Affine(self.params['W9'], self.params['b9'])
		self.layers['Relu9'] = Relu()
		self.layers['Dropout1'] = Dropout(0.5)

		self.layers['Affine2'] = Affine(self.params['W10'], self.params['b10'])
		self.layers['Relu10'] = Relu()
		self.layers['Dropout2'] = Dropout(0.5)

		self.layers['Affine3'] = Affine(self.params['W11'], self.params['b11'])
	...
	def gradient(self, x, y):
		...
		grads['W9'] = self.layers['Affine1'].dW
		grads['b9'] = self.layers['Affine1'].db
		grads['W10'] = self.layers['Affine2'].dW
		grads['b10'] = self.layers['Affine2'].db
		grads['W11'] = self.layers['Affine3'].dW
		grads['b11'] = self.layers['Affine3'].db
